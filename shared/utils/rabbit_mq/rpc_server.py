...
#             try:
#                 logger.info(f"RPC Server connecting to RabbitMQ (attempt {attempt + 1}/{max_retries})")
...
#                 logger.warning(f"RPC Server failed to connect (attempt {attempt + 1}/{max_retries}): {e}")
#                 if attempt < max_retries - 1:
#                     time.sleep(retry_delay)
#                 else:
#                     logger.error("RPC Server max retries exceeded")
#                     raise
#             except Exception as e:
#                 logger.error(f"RPC Server unexpected connection error: {e}")
#                 raise
...
#     def start_server(self, callback: Callable[[Dict[str, Any]], Dict[str, Any]]):
...
#                     logger.info(f"RPC request received: {request}")
                    
#                     # Process the request using the callback
...
#                     ch.basic_ack(delivery_tag=method.delivery_tag)
#                     logger.info(f"RPC response sent: {response}")
...
#             logger.info(f"RPC Server waiting for requests on queue: {self.queue_name}")
#             print(f"RPC Server started. Waiting for RPC requests on queue: {self.queue_name}")
